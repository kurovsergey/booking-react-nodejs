...
const offices_dto_1 = require("./dto/offices.dto");
...
let OfficesController = class OfficesController {
    ...
    async findOne(id, req) {
        ...
        if (req.user.role !== 'admin' && office.createdById !== req.user.id) {
            throw new common_1.ForbiddenException('You do not own this office');
        }
        return office;
    }
    async update(id, dto, req) {
        const office = await this.officesService.findOne(id);
        if (req.user.role !== 'admin' && office.createdById !== req.user.id) {
            throw new common_1.ForbiddenException('You can only update your own office');
        }
    ...
    async remove(id, req) {
        const office = await this.officesService.findOne(id);
        if (req.user.role !== 'admin' && office.createdById !== req.user.id) {
            throw new common_1.ForbiddenException('You can only delete your own office');
        }
...
__decorate([
    ...
    __metadata("design:paramtypes", [offices_dto_1.CreateOfficeDto, Object]),
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, offices_dto_1.UpdateOfficeDto, Object]),
    __metadata("design:returntype", Promise)
